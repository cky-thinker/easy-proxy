export class Message {
    ...
    encode(): Buffer {
        ...
        const buffer = Buffer.alloc(4 + 1 + 4 + tokenBuf.length + 4 + this.data.length);
...
export class MessageDecoder {
    ...
    decode(): Message | null {
        ...
        if (check !== Message.CHECK_VALUE) {
            ...
            const nextCheckIndex = this.buffer.indexOf(Buffer.from([0xF0, 0xF0, 0xF0, 0xF0]), 1);
            if (nextCheckIndex !== -1) {
        ...
        const token = this.buffer.toString('utf-8', offset, offset + tokenLength);
        offset += tokenLength;
        ...
        const data = Buffer.from(this.buffer.subarray(offset, offset + dataLength));
        offset += dataLength;
