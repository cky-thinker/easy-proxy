...
export class EasyProxyClient extends EventEmitter {
    ...
    private processConnect(msg: Message) {
        ...
        if (parts.length !== 2) {
            console.error(`EP>>ClientMng>> Proxy address '${proxyAddress}' is not valid`);
        ...
        if (isNaN(port)) {
            console.error(`EP>>ClientMng>> Proxy address '${proxyAddress}' port is not valid`);
            this.sendDisconnect(userId);
            return;
        }
        ...
        proxySocket.on('error', (err) => {
            console.error(`EP>>ClientMng>> Proxy socket '${proxyAddress}' connect fail, ${err.message}`);
            this.sendDisconnect(userId);
        });
    }

    private initProxyDataConnection(userId: string, proxySocket: net.Socket) {
        console.debug(`EP>>ClientProxy>> Proxy socket ${proxySocket.remoteAddress}:${proxySocket.remotePort} create success`);
        ...
        dataSocket.on('error', (err) => {
            console.debug(`EP>>ClientProxy>> Data socket error: ${err.message}`);
        });

        proxySocket.on('close', cleanup);
        proxySocket.on('error', (err) => {
            console.debug(`EP>>ClientProxy>> Proxy socket error: ${err.message}`);
        });
    }
    ...
    private retry() {
        ...
        setTimeout(() => {
            console.info(`try connect ${this.config.serverIp}:${this.config.serverPort}...`);
            this.connectMngServer();
        }, this.waitTime);
    }
