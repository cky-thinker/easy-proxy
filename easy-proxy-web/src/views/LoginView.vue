...
<div class="h-screen flex overflow-hidden bg-gray-50 lg:bg-white">
  ...
  <div
    class="hidden lg:flex lg:w-7/12 bg-gradient-to-br from-blue-600 to-indigo-900 flex-col items-center justify-center p-12 relative overflow-hidden">
    <div class="relative z-10 w-full max-w-2xl flex flex-col items-center">
      ...
      <div
        class="w-full max-w-lg aspect-[4/3] bg-white/5 backdrop-blur-sm rounded-3xl p-4 shadow-2xl border border-white/10">
        <LoginArchitecture />
      </div>
    </div>
    ...
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
      <div
        class="absolute -top-[20%] -left-[10%] w-[50%] h-[50%] bg-blue-500 rounded-full mix-blend-overlay filter blur-[100px] opacity-30">
      </div>
      <div
        class="absolute -bottom-[20%] -right-[10%] w-[50%] h-[50%] bg-indigo-500 rounded-full mix-blend-overlay filter blur-[100px] opacity-30">
      </div>
    </div>
  </div>
  ...
  <div class="w-full lg:w-5/12 flex items-center justify-center p-4 sm:p-6 lg:p-12 overflow-y-auto h-full">
    <div
      class="max-w-md w-full space-y-8 bg-white p-8 rounded-2xl shadow-xl lg:shadow-none border border-gray-100 lg:border-none">
      <div>
      ...
      <el-form class="mt-8 space-y-6" :model="loginForm" :rules="rules" ref="loginFormRef" @submit.prevent="handleLogin"
        ...
        <el-form-item v-if="captchaEnabled" label="验证码" prop="captchaCode">
          <div class="flex w-full gap-3">
            ...
            <div class="h-10 w-32 flex-shrink-0">
              <img v-if="captchaImage" :src="captchaImage.img" alt="验证码"
        ...
        <div v-if="errorMessage"
          class="p-3 rounded-md bg-red-50 text-red-600 text-sm text-center border border-red-100 flex items-center justify-center gap-2">
          <el-icon>
            <Warning />
          </el-icon>
          {{ errorMessage }}
        ...
        <el-form-item class="pt-2">
          <el-button type="primary" :loading="isLoading" native-type="submit"
            class="w-full !h-12 !text-lg !rounded-lg shadow-md hover:shadow-lg transition-all duration-300">
            登 录
          </el-button>
        </el-form-item>
      </el-form>
...
const handleLogin = async () => {
  if (captchaEnabled.value && !captchaImage.value) {
    errorMessage.value = '请点击右侧验证码获取'
    ElMessage.warning('请点击右侧验证码获取')
    return
  }
  ...
  try {
    ...
    } else if (captchaEnabled.value && !loginForm.value.captchaCode) {
    ...
    // 刷新验证码（仅当启用且为服务端返回错误时刷新）
    if (captchaEnabled.value && error?.response) {
