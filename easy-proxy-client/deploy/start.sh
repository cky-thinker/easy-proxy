...
if [ -f "app.jar" ]; then
    ...
    JAR_PATH=$(find . -name "easy-proxy-client-*-fat.jar" | head -n 1)
    if [ -z "$JAR_PATH" ]; then
        JAR_PATH=$(find ../target -name "easy-proxy-client-*-fat.jar" 2>/dev/null | head -n 1)
    fi
fi
...
MAIN_CLASS="com.cky.proxy.client.ProxyClient"
