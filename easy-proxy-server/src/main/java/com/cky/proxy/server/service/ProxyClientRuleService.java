...
public class ProxyClientRuleService {
    ...
    public List<ProxyClientRule> getProxyClientRules(Integer proxyClientId) {
        return proxyClientRuleMapper.selectList(
                new LambdaQueryWrapper<ProxyClientRule>().eq(ProxyClientRule::getProxyClientId, proxyClientId));
    }
    ...
    public List<ProxyClientRule> getAllProxyClientRules(String name, Integer serverPort, Integer proxyClientId,
        ...
        if (name != null && !name.isEmpty()) {
            wrapper.like(ProxyClientRule::getName, name);
        }
        if (serverPort != null) {
            wrapper.eq(ProxyClientRule::getServerPort, serverPort);
        }
        if (proxyClientId != null) {
            wrapper.eq(ProxyClientRule::getProxyClientId, proxyClientId);
        }
        if (enableFlag != null) {
            wrapper.eq(ProxyClientRule::getEnableFlag, enableFlag);
        }
    ...
    private void validateRuleNameUnique(Integer proxyClientId, String name, Integer excludeId) {
        if (name == null)
            return;
        LambdaQueryWrapper<ProxyClientRule> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(ProxyClientRule::getProxyClientId, proxyClientId).eq(ProxyClientRule::getName, name);
        if (excludeId != null) {
            wrapper.ne(ProxyClientRule::getId, excludeId);
        }
        boolean exists = !proxyClientRuleMapper.selectList(wrapper).isEmpty();
        if (exists)
            throw new RuntimeException("同客户端下规则名称已存在");
    }
    ...
    private void validateServerPortRangeAndUnique(Integer port, Integer excludeId) {
        ...
        LambdaQueryWrapper<ProxyClientRule> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(ProxyClientRule::getServerPort, port);
        if (excludeId != null) {
            wrapper.ne(ProxyClientRule::getId, excludeId);
        }
        boolean exists = !proxyClientRuleMapper.selectList(wrapper).isEmpty();
        if (exists)
            throw new RuntimeException("服务端口已被占用");
    }
    ...
    private void validateLimits(Integer limitConn, Integer limitRate) {
        if (limitConn != null && limitConn < 0)
            throw new RuntimeException("连接数限制不能为负数");
        if (limitRate != null && limitRate < 0)
            throw new RuntimeException("带宽限制不能为负数");
    }
